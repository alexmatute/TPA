...
class MXUPLOAD_File_Scan
{
	...
	/**
	 * MXUPLOAD_File_Scan constructor.
	 *
	...
	public function __construct($root_path, $timeout = 25.0, $paths_left = [])
	{
		...
		$this->instance   = MXUPLOAD_Admin_Main::get_instance();
	}
	...
	public function start()
	{
		...
		if (empty($this->paths_left)) {
			update_site_option('mxupload_file_scan', [
				'scan_finished' => false,
				'types'         => [],
			]);
		} else {
			$this->type_list = get_site_option('mxupload_file_scan');
		}
		...
		if (empty($this->paths_left)) {
			...
			update_site_option('mxupload_file_scan', $this->type_list);
		} else {
			error_log('File scan paused. Paths left: ' . implode(', ', $this->paths_left)); // Debug log
		}
	}
	...
	protected function is_excluded($path)
	{
		 ...
		 * @since  1.0
		 * @hook   mxupload_sync_exclusions
		 *
		 */
		$exclusions = apply_filters('mxupload_sync_exclusions', $this->exclusions);
